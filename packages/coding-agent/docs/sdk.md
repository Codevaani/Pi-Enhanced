...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@earendil-works/pie-coding-agent";
...
```bash
npm install @earendil-works/pie-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@earendil-works/pie-coding-agent";
...
The `Agent` class (from `@earendil-works/pie-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.pie/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.pie/extensions/`)
- Project skills:
  - `.pie/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.pie/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.pie/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@earendil-works/pie-coding-agent";
...
// Default: uses ~/.pie/agent/auth.json and ~/.pie/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@earendil-works/pie-coding-agent";
...
import { createAgentSession, defineTool } from "@earendil-works/pie-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.pie/agent/extensions/`, `.pie/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/pie-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/pie-coding-agent";
...
1. Global: `~/.pie/agent/settings.json`
2. Project: `<cwd>/.pie/settings.json`
