...
By default, `install` and `remove` write to user settings (`~/.pie/agent/settings.json`). Use `-l` to write to project settings (`.pie/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.pie/agent/npm/`.
- Project installs go under `.pie/npm/`.
...
- Cloned to `~/.pie/agent/git/<host>/<path>` (global) or `.pie/git/<host>/<path>` (project).
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@earendil-works/pie-ai`, `@earendil-works/pie-agent-core`, `@earendil-works/pie-coding-agent`, `@earendil-works/pie-tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.pie/agent`) and project (`.pie/`) scopes.
