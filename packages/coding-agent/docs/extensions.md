...
> **Placement for /reload:** Put extensions in `~/.pie/agent/extensions/` (global) or `.pie/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.pie/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.pie/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.pie/agent/extensions/*.ts` | Global (all projects) |
| `~/.pie/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.pie/extensions/*.ts` | Project-local |
| `.pie/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@earendil-works/pie-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@earendil-works/pie-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@earendil-works/pie-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@earendil-works/pie-coding-agent";
...
```typescript
import { isBashToolResult } from "@earendil-works/pie-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@earendil-works/pie-coding-agent";
...
```typescript
import { SessionManager } from "@earendil-works/pie-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@earendil-works/pie-tui";
...
```typescript
import { withFileMutationQueue } from "@earendil-works/pie-coding-agent";
...
**Important:** Use `StringEnum` from `@earendil-works/pie-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [ripgrep.ts](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/core/tools/ripgrep.ts) - `RipgrepToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@earendil-works/pie-coding-agent";
...
**Operations interfaces:** `ReadOperations`, `WriteOperations`, `EditOperations`, `BashOperations`, `LsOperations`, `RipgrepOperations`, `FindOperations`
...
```typescript
import { createBashTool } from "@earendil-works/pie-coding-agent";
...
```typescript
import { keyHint } from "@earendil-works/pie-coding-agent";
...
```typescript
import { Text, Component } from "@earendil-works/pie-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@earendil-works/pie-coding-agent";
import { matchesKey } from "@earendil-works/pie-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@earendil-works/pie-coding-agent";
