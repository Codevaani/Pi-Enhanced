...
describe("package commands", () => {
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".pie"), { recursive: true });
		writeFileSync(join(projectDir, ".pie", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".pie", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".pie"), { recursive: true });
		writeFileSync(join(projectDir, ".pie", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".pie", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".pie", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("pie install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "pie --help" or "pie install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: pie install <source> [-l]");
...
`,
		...
		Object.defineProperty(process, "execPath", {
			...
			try {
				...
				expect(recordedArgs).toEqual(
					expect.arrayContaining(["install", "-g", "--ignore-scripts", "--min-release-age=0", PACKAGE_NAME]),
				);
			} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
	...
	it("runs self-update when a newer release is available", async () => {
			const globalPrefix = join(tempDir, "global-prefix");
			const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "pi-coding-agent");
			const fakeNpmPath = join(tempDir, "fake-npm.cjs");
			const recordPath = join(tempDir, "self-update.json");
