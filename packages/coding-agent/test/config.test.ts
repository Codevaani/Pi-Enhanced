...
describe.skipIf(process.platform === "win32")("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@earendil-works/pie-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @earendil-works/pie-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@earendil-works/pie-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@earendil-works/pie-coding-agent")).toBe(
			"Update @earendil-works/pie-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@earendil-works/pie-coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@earendil-works/pie-coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @earendil-works/pie-coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@earendil-works/pie-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @earendil-works/pie-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@earendil-works/pie-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @earendil-works/pie-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@earendil-works/pie-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @earendil-works/pie-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@earendil-works/pie-coding-agent";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@earendil-works/pie-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@earendil-works/pie-coding-agent")).toContain(
