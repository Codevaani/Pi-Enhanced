...
import type {
	...
	AgentTool,
	ThinkingLevel,
} from "@earendil-works/pie-agent-core";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@earendil-works/pie-ai";
import {
	clampThinkingLevel,
...
export class AgentSession {
	...
	dispose(): void {
		...
		this._extensionRunner.invalidate(
			"This extension ctx is stale after session replacement or reload. Do not use a captured pie or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the ctx passed to withSession. For reload, do not use the old ctx after await ctx.reload().",
		);
	 ...
	 * - Handles extension commands (registered via pie.registerCommand) immediately, even during streaming
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// Extension commands manage their own LLM interaction via pie.sendMessage()
	...
	private _buildRuntime(options: {
		...
		const defaultActiveToolNames = this._baseToolsOverride
			...
			: ["read", "bash", "edit", "write", "ripgrep", "todo", "web_search"];
		...
		// and registers their tools as built-in pie tools.
		this._initMcp();
	}
