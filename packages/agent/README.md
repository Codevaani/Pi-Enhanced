# @earendil-works/pie-agent-core

Stateful agent with tool execution and event streaming. Built on `@earendil-works/pie-ai`.

## Installation

```bash
npm install @earendil-works/pie-agent-core
```
...
```typescript
declare module "@earendil-works/pie-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/pie-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/pie-agent-core";
